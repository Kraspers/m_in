  ...
  (function(){
    ...
    window.closeFolderCtx=function(e){const ov=document.getElementById('folder-ctx-overlay'); if(!ov)return; if(e&&e.target!==ov&&e.target!==document.getElementById('chatlist-ctx-blur-bg'))return; closeFolderCtxClean();};
    window.closeFolderCtxClean=function(){ const ov=document.getElementById('folder-ctx-overlay'); const rw=document.getElementById('folder-ctx-row-wrap'); const mn=document.getElementById('folder-ctx-menu'); if(!ov||!rw||!mn)return; ov.style.pointerEvents='none'; ov.style.transition='opacity 0.30s ease'; ov.style.opacity='0'; rw.style.transition='transform 0.28s cubic-bezier(0.36,0,0.66,0)'; rw.style.transform=`translateY(${folderMsgOriginalOffset}px)`; mn.style.transition='transform 0.28s cubic-bezier(0.36,0,0.66,0),opacity 0.24s ease'; mn.style.transform='scale(0.86)'; mn.style.opacity='0'; folderCtxCloseTimer=setTimeout(()=>{ ov.classList.remove('open'); ov.style.transition='';ov.style.opacity='';ov.style.pointerEvents=''; rw.style.transition='';rw.style.transform=''; mn.style.transition='';mn.style.transform='';mn.style.opacity=''; setTimeout(()=>{rw.innerHTML='';folderCtxCloseTimer=null;},100); },310); };
