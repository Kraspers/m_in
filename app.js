...
(async function initBackend(){
  ...
  function renderMenuChatRow(c,{selected=false,check=false,owner=false}={}){const preview=c.preview||(c.username?('@'+c.username):(c.statusText||'')); const suffix=owner?'<span class="owner-pill">Владелец</span>':(check?`<span class="pick-check">${selected?'✓':''}</span>`:''); return `<button class="chat-row chat-row-item mi-chat-row ${selected?'selected':''}" data-uid="${esc(c.id||'')}">${rowAvatarHtml(c)}<div style="flex:1;min-width:0;"><span class="chat-row-name" style="color:#fff;font-size:16px;font-weight:600;">${nameWithVerificationHtml(c.name||c.username||'Пользователь',!!c.verified)}</span>${preview?`<span class="chat-row-preview">${esc(preview)}</span>`:''}</div>${suffix}</button>`;}
  window.openNewChatMenu=function(){const w=document.getElementById('new-chat-wrap'); if(!w)return; const lbl=document.getElementById('new-chat-close-label'); if(lbl) lbl.textContent='Создать группу'; w.style.display='flex'; requestAnimationFrame(()=>w.classList.add('open')); document.getElementById('new-chat-main-view').style.display='block'; document.getElementById('group-pick-view').style.display='none'; document.getElementById('group-details-view').style.display='none'; searchStartedChats(''); setTimeout(()=>document.getElementById('new-chat-search')?.focus(),120);};
  window.closeNewChatMenu=function(){const w=document.getElementById('new-chat-wrap'); if(!w)return; w.classList.remove('open'); setTimeout(()=>{if(!w.classList.contains('open'))w.style.display='none';},380);};
  ...
  window.openGroupMembersMenu=function(group=null){
    ...
    w.style.display='flex'; requestAnimationFrame(()=>w.classList.add('open'));
  };
  window.closeGroupMembersMenu=function(){const w=document.getElementById('group-members-wrap'); if(!w)return; w.classList.remove('open'); setTimeout(()=>{if(!w.classList.contains('open'))w.style.display='none';},380);};
