  ...
  async function openMinLink(url,apiFn){
    ...
    if(info.type==='group'){
      try{
        const data=await call(`/groups/join/${encodeURIComponent(info.code)}`,{method:'POST'});
        if(data&&data.group){
          usersMap.set(data.group.id,{...(usersMap.get(data.group.id)||{}),...data.group});
          if(window.openChatWith) await window.openChatWith(data.group.id);
          return true;
        }
      }catch(_){
        try{
          const data=await call(`/public-group?code=${encodeURIComponent(info.code)}`);
          if(data&&data.group) showTopToast(data.group.name||'Группа');
        }catch(__){}
      }
      return true;
    }
    try{
      ...
      if(user&&user.id){
        ...
        if(me&&user.id===me.id) openFavorites();
        else openUserProfileView(user);
      }else alert(t('user_not_found'));
    }catch(_){ alert(t('user_not_found')); }
    return true;
  }
  async function loadMinPreview(url,apiFn){
    const info=parseMinLink(url);
    if(!info) return null;
    const call=apiFn||api||backendApi;
    if(info.type==='group'){
  ...
  (function(){
    ...
    function ensureLegalContent(){
      const privacy=document.getElementById('legal-privacy');
      const terms=document.getElementById('legal-terms');
      if(!privacy||!terms||privacy.dataset.ready==='1') return;
      privacy.innerHTML=`<div class="legal-hero"><h1>Политика конфиденциальности</h1><p>MIN создан для личного и безопасного общения. Мы не продаём данные, не используем переписки для рекламы и не собираем содержимое сообщений, список чатов или медиа для аналитики.</p></div><div class="legal-card"><h2>1. Что обрабатывается</h2><p>Для работы аккаунта нужны только базовые данные: логин, имя профиля, пароль в защищённом виде, VPSC-код, настройки профиля и техническая информация о входе. Переписка, чаты и файлы обрабатываются только для доставки и синхронизации внутри приложения.</p><h2>2. Шифрование и защита</h2><p>Данные передаются и хранятся в защищённом виде. Сообщения, медиа и служебные ключи не предназначены для просмотра третьими лицами. Доступ к техническим системам ограничен и используется только для безопасности и стабильности сервиса.</p><h2>3. VPSC-код</h2><p>VPSC-код — ключ к аккаунту. Не отправляйте его другим людям и не вводите на сторонних сайтах. MIN не просит VPSC-код в чатах или личных сообщениях.</p><h2>4. Свобода общения</h2><p>MIN уважает свободу слова и приватность пользователей. Мы не читаем личные разговоры ради рекламы, рейтингов или рекомендаций. Ограничения применяются только против спама, взломов, угроз и действий, которые вредят безопасности людей или сервиса.</p><h2>5. Передача данных</h2><p>Мы не продаём и не передаём личные данные рекламным сетям. Передача возможна только если это требуется законом, защитой пользователей, расследованием атак или работой необходимой инфраструктуры.</p><h2>6. Управление аккаунтом</h2><p>Вы можете менять пароль, защищать доступ через VPSC и удалить аккаунт в настройках. После удаления технические следы могут временно оставаться в резервных копиях только для безопасности и восстановления сервиса.</p></div>`;
      terms.innerHTML=`<div class="legal-hero"><h1>Условия использования</h1><p>MIN — мессенджер для безопасного общения и свободы слова. Пользуясь приложением, вы принимаете эти правила и отвечаете за безопасность своего аккаунта.</p></div><div class="legal-card"><h2>1. Аккаунт</h2><p>Храните пароль и VPSC-код в тайне. Не передавайте аккаунт другим людям, не выдавайте себя за другого пользователя и не пытайтесь получить доступ к чужим данным.</p><h2>2. Свобода слова</h2><p>В MIN можно свободно общаться, спорить и выражать свою позицию. Но свобода слова не разрешает угрозы, травлю, мошенничество, доксинг, спам, вредоносные файлы и действия, нарушающие закон или безопасность других людей.</p><h2>3. Безопасность сервиса</h2><p>Запрещено атаковать приложение, обходить ограничения, автоматизировать спам, взламывать аккаунты, публиковать вредоносные ссылки или мешать работе MIN.</p><h2>4. Ваш контент</h2><p>Ваши сообщения и файлы остаются вашими. Приложение технически обрабатывает их только для отправки, получения, синхронизации и защиты, в зашифрованном и безопасном виде.</p><h2>5. Ограничения</h2><p>Если пользователь нарушает правила или угрожает безопасности, MIN может ограничить функции, удалить опасный контент или заблокировать аккаунт.</p><h2>6. Работа приложения</h2><p>Мы стараемся поддерживать стабильную работу MIN, но функции могут обновляться, временно отключаться или меняться ради безопасности, исправления ошибок и развития сервиса.</p></div>`;
      privacy.dataset.ready='1';
    }
    ...
    function renderChatMessages(items,firstUnreadMessageId=''){
      ...
      const rows=items.map((m,idx)=>{
        ...
        const senderUser=usersMap.get(m.fromUserId)||{};
        const senderNameHtml=showGroupSender?`<button type="button" class="group-sender-name" data-user-id="${esc(m.fromUserId)}">${nameWithVerificationHtml(displayNameForMessageUser(m.fromUserId),!!senderUser.verified)}</button>`:'';
    ...
    (function initMinMenu(){
      ...
      const qrWrap=document.getElementById('min-menu-qr-wrap');
      if(!trigger||!wrap||!bg||!closeBtn||!logo||!dock||!title) return;
      if(qr){
        qr.addEventListener('load',()=>qrWrap&&qrWrap.classList.remove('qr-failed'));
        qr.addEventListener('error',()=>qrWrap&&qrWrap.classList.add('qr-failed'));
        qr.src=`https://api.qrserver.com/v1/create-qr-code/?size=512x512&data=${encodeURIComponent(location.origin)}`;
      }
      let lastTap=0;
      function openMenu(){
...
(async function initBackend(){
  ...
  let groupPickMode='create', groupPickedIds=new Set(), groupAvatarDataUrl='', groupBannerDataUrl='', currentGroupForInvite=null, groupCreateInFlight=false;
  ...
  async function loadPickList(){const box=document.getElementById('group-pick-list'); if(!box)return; const excluded=new Set([me&&me.id,...((currentGroupForInvite&&currentGroupForInvite.members)||[]).map(m=>m&&m.id),...groupPickedIds].filter(Boolean)); const allowed=c=>c&&c.id&&!c.isGroup&&!excluded.has(c.id); const cached=Array.from(usersMap.values()).filter(allowed); renderPickItems(cached); const data=await api('/chats/started'); const items=(data.items||[]).filter(allowed); items.forEach(c=>usersMap.set(c.id,c)); renderPickItems(items);}
...
${url}`; const ids=[...groupPickedIds]; await Promise.all(ids.map(id=>api('/messages',{method:'POST',body:JSON.stringify({toUserId:id,text})}).catch(()=>null))); closeNewChatMenu(); showTopToast(t('sent')); };
  ...
  window.createGroupNow=async function(){
    if(groupCreateInFlight) return;
    const btn=document.getElementById('create-group-btn');
    const name=document.getElementById('group-name-input').value.trim();
    const bio=(document.getElementById('group-bio-input')?.value||'').trim().slice(0,110);
    if(!name){alert(t('group_name'));return;}
    ...
    try{
      const res=await api('/groups',{method:'POST',body:JSON.stringify({name,bio,bannerDataUrl:groupBannerDataUrl,avatarDataUrl:groupAvatarDataUrl,memberIds:[...groupPickedIds],requestId})});
      closeNewChatMenu();
      if(res.group) window.openChatWith(res.group.id);
      groupAvatarDataUrl=''; groupBannerDataUrl='';
      document.getElementById('group-name-input').value='';
      const bioEl=document.getElementById('group-bio-input'); if(bioEl) bioEl.value='';
      const bannerImg=document.getElementById('group-banner-img'); if(bannerImg){bannerImg.src='';bannerImg.style.display='none';}
  ...
  window.copyCurrentGroupInvite=async function(){ if(!currentGroupForInvite)return; const url=`${location.origin}/m-in/group/${currentGroupForInvite.inviteCode}`; try{await navigator.clipboard.writeText(url); showTopToast('Скопировано');}catch(_){prompt('Ссылка на вступление',url);} };
