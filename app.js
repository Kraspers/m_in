...
(async function initBackend(){
  ...
  async function loadPickList(){const box=document.getElementById('group-pick-list'); if(!box)return; const cached=Array.from(usersMap.values()).filter(c=>c&&c.id&&!c.isGroup&&(!currentGroupForInvite||!(currentGroupForInvite.members||[]).some(m=>m.id===c.id))); if(cached.length) renderPickItems(cached); const data=await api('/chats/started'); const items=(data.items||[]).filter(c=>!currentGroupForInvite||!(currentGroupForInvite.members||[]).some(m=>m.id===c.id)); items.forEach(c=>usersMap.set(c.id,c)); renderPickItems(items);}
